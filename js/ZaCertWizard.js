...
function ZaCertWizard(parent) {
    ...
    this.stepChoices = [ {
        label : com_zimbra_cert_manager.CERT_WIZARD_TABT_selectServer,
        value : ZaCertWizard.STEP_SELECT_SERVER
    }, {
        label : com_zimbra_cert_manager.CERT_WIZARD_TABT_useroption,
        value : ZaCertWizard.STEP_USER_OPTION
    }, {
        label : com_zimbra_cert_manager.CERT_WIZARD_TABT_gencsr,
        value : ZaCertWizard.STEP_GEN_CSR
    }, {
        label : com_zimbra_cert_manager.CERT_WIZARD_TABT_uploadCert,
        value : ZaCertWizard.STEP_UPLOAD_CERT
    }, {
        label : com_zimbra_cert_manager.CERT_WIZARD_TABT_installCert,
        value : ZaCertWizard.STEP_INSTALL_CERT
    }, {
        label : com_zimbra_cert_manager.CERT_WIZARD_TABT_downloadCSR,
        value : ZaCertWizard.STEP_DOWNLOAD_CSR
    }, {
        label : com_zimbra_cert_manager.CERT_WIZARD_TABT_reviewCSR,
        value : ZaCertWizard.STEP_CSR_CONFIRM
    } ];
 ...
 * finish)
 */
ZaCertWizard.prototype.popup = function(loc) {
...
ZaCertWizard.prototype.finishWizard = function() {
    try {
        ...
        // 1) install the cert
        // 2) Upon the successful install, the cert tab information will be
        // updated
        ...
        var params = {
            ...
            // allserver: (this._containedObject[ZaCert.A_target_server] ==
            // ZaCert.ALL_SERVERS) ? 1 : 0,
            callback : callback
        }
...
ZaCertWizard.getCause = function(detailMsg) {
...
ZaCertWizard.prototype.installCallback = function(resp) {
    ...
    try {
        if (resp._isException) {
            ...
            throw new AjxException(com_zimbra_cert_manager.CERT_INSTALL_STATUS_1 + ": " + ZaCertWizard.getCause(detailMsg), "ZaCertWizard.prototype.installCallback", AjxException.UNKNOWN_ERROR,
                    detailMsg);
            ...
            if (installResponse) {
                ...
                if (controller instanceof ZaCertViewController) {
                    controller.show(ZaCert.getCerts(ZaApp.getInstance(), this._containedObject[ZaCert.A_target_server]), this._containedObject[ZaCert.A_target_server]);
                }
                ZaCertWizard.INSTALL_STATUS = -1;
            } else {
        ...
        // _handleException(ex, "ZaCertWizard.prototype.installCallback", null,
        // false);
    }
}

ZaCertWizard.prototype._uploadCallback = function(status, uploadResults) {
    var cStep = this._containedObject[ZaModel.currentStep];
    ...
    if ((status == AjxPost.SC_OK) && (uploadResults != null) && (uploadResults.length > 0)) {
        ...
        for (var i = 0; i < uploadResults.length; i++) {
            ...
            // comment this out as these are coming back as
 ...
 */
ZaCertWizard.prototype.setUploadManager = function(uploadManager) {
...
ZaCertWizard.prototype.goNext = function() {
    ...
    if (cStep == ZaCertWizard.STEP_SELECT_SERVER) {
        ...
        this._containedObject.initCSR(ZaCert.getCSR(ZaApp.getInstance(), this._containedObject[ZaCert.A_target_server], type));
        ...
        if (cn == null || cn.match(cn_regEx) == null) {
            // show error msg
            ZaApp.getInstance().getCurrentController().popupErrorDialog(AjxMessageFormat.format(com_zimbra_cert_manager.CERT_CN_INVALID, cn || "Current CN "));
            return false;
        }
        ...
        for (var i = 0; i < subjAltNames.length; i++) {
            if (subjAltNames[i].match(san_regEx) == null) {
                ZaApp.getInstance().getCurrentController().popupErrorDialog(AjxMessageFormat.format(com_zimbra_cert_manager.CERT_SUBJ_ALT_NAME_INVALID, subjAltNames[i] || "SubjectAltName " + i));
                return false;
            }
        }
        ...
        try {
            if ((!this._containedObject[ZaCert.A_csr_exists]) || (this._containedObject[ZaCert.A_force_new_csr] == 'TRUE')) {
                if (!this._containedObject[ZaCert.A_type_self]) {
                    ZaCert.genCSR(ZaApp.getInstance(), this._containedObject.attrs, type, true, this._containedObject[ZaCert.A_target_server], this._containedObject[ZaCert.A_keysize],
        ...
        if (this._containedObject[ZaCert.A_type_comm]) {
            ...
            // No same file name is allowed due to the server limitation -
            // server only return the filename
                ...
                if (inputEls[i].type == "file") {
                    ...
                    if (v != null && v.length != 0) {
                        if (ZaUtil.findValueInArray(filenameArr, v) != -1) {
                            ZaApp.getInstance().getCurrentController().popupErrorDialog(com_zimbra_cert_manager.dupFileNameError + v);
                            return;
                        }
                        filenameArr.push(v);
                    }
            ...
            try {
                ZaUploader.upload.call(this, certUploadCallback, [ZaCertWizard.certAttachmentInputId, ZaCertWizard.rootCAAttachmentInputId, ZaCertWizard.intermediateCAAttachmentInputId], ZaCertWizard.CertUploadFormId);
            } catch (err) {
...
ZaCertWizard.getFileName = function(fullPath) {
    ...
    var lastIndex = 0;
    /*
     * if (AjxEnv.isWindows) { lastIndex = fullPath.lastIndexOf("\\") ; }else{
...
ZaCertWizard.prototype.goPrev = function() {
    ...
    } else if (cStep == ZaCertWizard.STEP_GEN_CSR || cStep == ZaCertWizard.STEP_CSR_CONFIRM) {
 ...
 *            object to display
 */
ZaCertWizard.prototype.setObject = function(entry) {
...
ZaCertWizard.getUploadFormHtml = function() {
    ...
    html[idx++] = ZaCertWizard.certAttachmentInputId;
    html[idx++] = "' name='certFile' size='40' /></td><td></td></tr>";
    html[idx++] = "<tr><td>" + com_zimbra_cert_manager.CERT_upload_root_CA + "</td>";
    html[idx++] = "<td><input type=file id='";
    html[idx++] = ZaCertWizard.rootCAAttachmentInputId;
    html[idx++] = "' name='rootCA' size='40' /></td><td></td></tr>";
...
ZaCertWizard.getIntermediaCAUploadInput = function() {
    ...
    html[idx++] = "' name='intermediateCA' size='40' /></td>";
    html[idx++] = "<td><span style='padding-left:5px; color: blue; text-decoration: underline; cursor: default;' " + " onmouseout='this.style.cursor=\"default\"' "
            + " onmouseover='this.style.cursor=\"pointer\"'" + " onclick='ZaCertWizard.removeIntermediaCAInput(this);' >" + com_zimbra_cert_manager.Remove_IntermediateCA_Label + "</span></td>";
...
ZaCertWizard.onRepeatRemove = function(index, form) {
...
ZaCertWizard.myXFormModifier = function(xFormObject) {
    ...
    var case_select_server = {
        ...
        numCols : 2,
        colSizes : [ "100px", "*" ],
        tabGroupKey : ZaCertWizard.STEP_SELECT_SERVER,
        caseKey : ZaCertWizard.STEP_SELECT_SERVER,
        align : _LEFT_,
        valign : _TOP_,
        width : "80%",
    ...
    var case_select_server_items = [ {
        ...
        type : _OUTPUT_,
        colSpan : 2,
        value : com_zimbra_cert_manager.CERT_select_server
    }, {
        type : _SPACER_,
        height : 10
    }, {
    ...
    /*
     * , { type:_CHECKBOX_, ref: ZaCert.A_target_all_servers , label:
     * com_zimbra_cert_manager.all_servers}
     */
    ];
    case_select_server.items = case_select_server_items;
    cases.push(case_select_server);

    var case_user_options = {
        type : _CASE_,
        numCols : 2,
        colSizes : [ "25px", "*" ],
        tabGroupKey : ZaCertWizard.STEP_USER_OPTION,
        caseKey : ZaCertWizard.STEP_USER_OPTION,
        align : _LEFT_,
        valign : _TOP_,
        width : "80%",
        cssStyle : "padding-left:50px;"
    ...
    var case_user_options_items = [ {
        ...
        type : _OUTPUT_,
        colSpan : 2,
        value : com_zimbra_cert_manager.CERT_select_option
    }, {
        type : _SPACER_,
        height : 10
    }, {
        type : _RADIO_,
        groupname : "install_type",
        ref : ZaCert.A_type_self,
        label : com_zimbra_cert_manager.CERT_self_signed,
        enableLabelFor : true,
        visibilityChecks : [],
        enableDisableChecks : [],
        ...
        onChange : function(value, event, form) {
            value = true;
            if (window.console && window.console.log)
                console.log("Self Install - onChange value = " + value);
        ...
        updateElement : function(newValue) {
            if (window.console && window.console.log)
                console.log("Self Install - UpdateElement newValue = " + newValue);
            this.getElement().checked = (newValue == true);
        }
    }, {
        type : _RADIO_,
        groupname : "install_type",
        ref : ZaCert.A_type_csr,
        label : com_zimbra_cert_manager.CERT_gen_csr,
        enableLabelFor : true,
        visibilityChecks : [],
        enableDisableChecks : [],
        labelLocation : _RIGHT_,
        ...
        onChange : function(value, event, form) {
            value = true;
            if (window.console && window.console.log)
                console.log("Gen CSR - onChange value = " + value);
            this.setInstanceValue(value);
        ...
        updateElement : function(newValue) {
            if (window.console && window.console.log)
                console.log("Gen CSR Install - UpdateElement newValue = " + newValue);
            this.getElement().checked = (newValue == true);
        }
    }, {
        type : _RADIO_,
        groupname : "install_type",
        ref : ZaCert.A_type_comm,
        visibilityChecks : [],
        enableDisableChecks : [],
        updateElement : function(newValue) {
            if (window.console && window.console.log)
                console.log("Comm Install - UpdateElement newValue = " + newValue);
            this.getElement().checked = (newValue == true);
        },
        onChange : function(value, event, form) {
            value = true;
            if (window.console && window.console.log)
                console.log("Comm Install - onChange value = " + value);
            this.setInstanceValue(value);
            this.setInstanceValue(!value, ZaCert.A_type_self);
            this.setInstanceValue(!value, ZaCert.A_type_csr);
        },
        label : com_zimbra_cert_manager.CERT_comm_signed,
        enableLabelFor : true,
        labelLocation : _RIGHT_,
        align : _LEFT_
    }, {
    ...
    var case_gen_csr = {
        ...
        numCols : 2,
        colSizes : [ "200px", "*" ],
        tabGroupKey : ZaCertWizard.STEP_GEN_CSR,
        caseKey : ZaCertWizard.STEP_GEN_CSR,
        align : _LEFT_,
        valign : _TOP_
    };
    ...
    var case_upload_cert = {
        type : _CASE_,
        numCols : 1,
        colSizes : [ "450px" ],
        tabGroupKey : ZaCertWizard.STEP_UPLOAD_CERT,
        caseKey : ZaCertWizard.STEP_UPLOAD_CERT,
        items : [ {
            type : _GROUP_,
            numCols : 2,
            colSpan : "*",
            ...
            items : [ {
                type : _OUTPUT_,
                value : com_zimbra_cert_manager.CERT_uploadTitle,
                align : _LEFT_
            }, {
                type : _OUTPUT_,
                value : ZaCertWizard.getUploadFormHtml()
            }, {
                type : _SPACER_,
                height : 10
            } ]
    ...
    var case_install_cert = {
        type : _CASE_,
        numCols : 2,
        colSizes : [ "150px", "300px" ],
        tabGroupKey : ZaCertWizard.STEP_INSTALL_CERT,
        caseKey : ZaCertWizard.STEP_INSTALL_CERT,
        align : _LEFT_,
        valign : _TOP_
    };
    ...
    var case_download_csr = {
        ...
        numCols : 1,
        colSizes : [ "*" ],
        tabGroupKey : ZaCertWizard.STEP_DOWNLOAD_CSR,
        caseKey : ZaCertWizard.STEP_DOWNLOAD_CSR,
        align : _LEFT_,
        valign : _TOP_,
        items : [ {
            type : _GROUP_,
            ...
            type : _OUTPUT_,
            value : com_zimbra_cert_manager.CSR_download_msg_1
        }, {
            type : _SPACER_,
            height : 10
        }, {
    ...
    var case_csr_confirm = {
        ...
        numCols : 1,
        colSizes : [ "*" ],
        tabGroupKey : ZaCertWizard.STEP_CSR_CONFIRM,
        caseKey : ZaCertWizard.STEP_CSR_CONFIRM,
        align : _LEFT_,
        valign : _TOP_,
        items : [ {
            type : _GROUP_,
            ...
            items : [ {
                ...
                type : _OUTPUT_,
                value : com_zimbra_cert_manager.CSR_REVIEW,
                colSpan : 2
            }, {
                type : _SPACER_,
                height : 10
            ...
            items : [ {
                ...
                type : _OUTPUT_,
                value : com_zimbra_cert_manager.CSR_NON_EXISTS_MSG
            } ]
        } ]
    }
